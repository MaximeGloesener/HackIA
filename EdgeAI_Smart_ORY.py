...
def authentification():
	...
	ALLOWED = ['Sidi']
	#
	face_model = torch.load('models/facenet.pt').to(device)
...
def fire_detection():
	device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
	fire_model = torch.load('models/firenet.pt').to(device)
	...
	classes = ['Fire','Start fire','No fire']
	#
	cam_port = os.path.join(BASE_PATH, "tests/firetest.mp4")
	cam = cv2.VideoCapture(cam_port)
	#
	prev_frame_time = 0
	new_frame_time = 0
...
def suspect_localisation():
        print("testtttttttt")
        os.system("cd yolov7; python detect.py --weights ORV7.pt --source 0")
...
def suspect_localisation2():
	...
	classes = []
	weights = os.path.join(BASE_PATH, 'models/yolov5s.pt')
	device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
...
def action_recognition():
	global camera,frame_queue,sample_length,data,test_pipeline,model,result_queue,device_name,average_size,labels,threshold
	threshold = 0
	average_size = 1
	...
	config_file = os.path.join(BASE_PATH, 'models/tsm/tsm_k400_pretrained_r50_1x1x8_25e_hmdb51_rgb.py')
	checkpoint_file = os.path.join(BASE_PATH, 'models/tsm/tsm_k400_pretrained_r50_1x1x8_25e_hmdb51_rgb_20210630-10c74ee5.pth')
	model = init_recognizer(config_file, checkpoint_file, device=device)
	print('model loaded')
	data = dict(img_shape=None, modality='RGB', label=-1)
	labels = list(map(str.strip,open( os.path.join(BASE_PATH, 'models/tsm/label_map.txt')).readlines()))
	camera_path = os.path.join(BASE_PATH,'./tests/actiontest.mp4')
